...
public class Hand {
	...
	public static Hand PickBestHand(ArrayList<Hand> Hands) throws HandException{
		...
		Hand second = Hands.get(1);
		for (int i = 1; i < Hands.size(); i++){
			if (Hands.get(i).getHandScore().getHandStrength() > highest.getHandScore().getHandStrength()) {
				highest = Hands.get(i);		
			}
			if (highest.getHandScore().getHandStrength() > Hands.get(i).getHandScore().getHandStrength() &&
					second.getHandScore().getHandStrength() < Hands.get(i).getHandScore().getHandStrength()) {
				
				second = Hands.get(i);
			}
		}
		
		if (!(highest.getHandScore().getHandStrength() > second.getHandScore().getHandStrength())){
			throw new HandException(highest);
		}
		else {
			return highest;
		}
	...
	static boolean isHandNRFlush(Hand h, HandScore hs) {
		boolean IsFlush = false;
		boolean flush = isHandFlush(h.getCardsInHand());
			if (flush){
				if (h.getCardsInHand().get(0).geteRank() == eRank.TEN && h.getCardsInHand().get(1).geteRank() == eRank.JACK && h.getCardsInHand().get(2).geteRank() == eRank.QUEEN && h.getCardsInHand().get(3).geteRank() == eRank.KING && h.getCardsInHand().get(4).geteRank() == eRank.ACE) {
					IsFlush = true;
				}
